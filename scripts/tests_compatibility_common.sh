...
start_node() {
	...
	if [ "$print_log" = true ]; then
		...
		./local_node.sh -y >/tmp/nowad.log 2>&1 &
	fi
	NODE_PID=$!
	popd >/dev/null
}
...
wait_for_node() {
	...
	local timeout=60
	local elapsed=0
	echo "Waiting for nowad node to be ready..."
	...
	if [ $elapsed -ge $timeout ]; then
		...
		tail -20 /tmp/nowad.log 2>/dev/null || echo "No nowad logs found"
		exit 1
	fi
}
