...
LOGLEVEL="info"
# Set dedicated home directory for the nowad instance
CHAINDIR="$HOME/.nowad"
...
Options:
  ...
  --mnemonic-file PATH     Where to write mnemonics YAML (default: \$HOME/.nowad/mnemonics.yaml)
...
while [[ $# -gt 0 ]]; do
  ...
  case $key in
    ...
    -n)
      ...
      ;;
    --no-install)
      echo "Flag --no-install passed -> Skipping installation of the nowad binary."
      install=false; shift
      ;;
    --remote-debugging)
...
add_genesis_funds() {
  local keyname="$1"
  nowad genesis add-genesis-account "$keyname" 1000000000000000000000atest --keyring-backend "$KEYRING" --home "$CHAINDIR"
}
...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
  ...
  nowad config set client chain-id "$CHAINID" --home "$CHAINDIR"
  nowad config set client keyring-backend "$KEYRING" --home "$CHAINDIR"

  # ---------------- Validator key ----------------
  VAL_KEY="mykey"
  VAL_MNEMONIC="gesture inject test cycle original hollow east ridge hen combine junk child bacon zero hope comfort vacuum milk pitch cage oppose unhappy lunar seat"
  echo "$VAL_MNEMONIC" | nowad keys add "$VAL_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$CHAINDIR"
  ...
  echo "$VAL_MNEMONIC" | nowad init $MONIKER -o --chain-id "$CHAINID" --home "$CHAINDIR" --recover
  ...
  jq '.app_state["bank"]["denom_metadata"]=[{"description":"The native staking token for nowad.","denom_units":[{"denom":"atest","exponent":0,"aliases":["attotest"]},{"denom":"test","exponent":18,"aliases":[]}],"base":"atest","display":"test","name":"Test Token","symbol":"TEST","uri":"","uri_hash":""}]' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
  ...
  nowad genesis add-genesis-account "$VAL_KEY" 100000000000000000000000000atest --keyring-backend "$KEYRING" --home "$CHAINDIR"
  ...
  for ((i=0; i<${#dev_mnemonics[@]}; i++)); do
    ...
    echo "$mnemonic" | nowad keys add "$keyname" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$CHAINDIR"
  ...
  if [[ "$ADDITIONAL_USERS" -gt 0 ]]; then
    ...
    for ((i=0; i<ADDITIONAL_USERS; i++)); do
      ...
      mnemonic_out="$(nowad keys add "$keyname" --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$CHAINDIR" 2>&1)"
  ...
  # --------- Finalize genesis ---------
  nowad genesis gentx "$VAL_KEY" 1000000000000000000000atest --gas-prices ${BASEFEE}atest --keyring-backend "$KEYRING" --chain-id "$CHAINID" --home "$CHAINDIR"
  nowad genesis collect-gentxs --home "$CHAINDIR"
  nowad genesis validate-genesis --home "$CHAINDIR"
